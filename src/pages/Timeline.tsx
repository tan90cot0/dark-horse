...
import { Calendar, Plus, Heart, X, Save, Edit, Image as ImageIcon, ChevronLeft, ChevronRight } from 'lucide-react';
import dataService, { TimelineEvent } from '../utils/dataService';
import { useNotification } from '../context/NotificationContext';
import { motion, AnimatePresence } from 'framer-motion';

// Simple Image Component
const TimelineImage = React.memo(({ event, className, onImageClick }: {
  event: TimelineEvent;
  className: string;
...
// Component to display key additional memory fields
const AdditionalMemoryFields = React.memo(({ event }: { event: TimelineEvent }) => {
  // Simplified fields list based on actual diary.json data - focusing on most common fields
  const allFields = [
    // Tier 1 - Most Important (commonly found fields)
    ...
    // Tier 2 - Additional Details (common fields)
    { key: 'memorable_quote', label: 'Quote', icon: '💬', color: 'text-cyan-400 bg-cyan-500/20', tier: 2 },
    { key: 'famous_quote', label: 'Famous Quote', icon: '💬', color: 'text-cyan-400 bg-cyan-500/20', tier: 2 },
    { key: 'saved_message', label: 'Saved Message', icon: '📱', color: 'text-cyan-400 bg-cyan-500/20', tier: 2 },
    { key: 'emotional_impact', label: 'Impact', icon: '❤️', color: 'text-red-400 bg-red-500/20', tier: 2 },
    { key: 'companions', label: 'With', icon: '👥', color: 'text-indigo-400 bg-indigo-500/20', tier: 2 },
    { key: 'food', label: 'Food', icon: '🍕', color: 'text-orange-400 bg-orange-500/20', tier: 2 },
    { key: 'food_choice', label: 'Food Choice', icon: '🍕', color: 'text-orange-400 bg-orange-500/20', tier: 2 },
    { key: 'adventure', label: 'Adventure', icon: '🚀', color: 'text-purple-400 bg-purple-500/20', tier: 2 },
    { key: 'gesture', label: 'Gesture', icon: '🤝', color: 'text-pink-400 bg-pink-500/20', tier: 2 },
    { key: 'celebration_type', label: 'Celebration', icon: '🎉', color: 'text-yellow-400 bg-yellow-500/20', tier: 2 },
    { key: 'first_experience', label: 'First', icon: '🌟', color: 'text-yellow-400 bg-yellow-500/20', tier: 2 },
    { key: 'firsts', label: 'Firsts', icon: '🌟', color: 'text-yellow-400 bg-yellow-500/20', tier: 2 },
    { key: 'activities', label: 'Activities', icon: '🎮', color: 'text-blue-400 bg-blue-500/20', tier: 2 },
    { key: 'milestone', label: 'Milestone', icon: '🏆', color: 'text-yellow-400 bg-yellow-500/20', tier: 2 },
    { key: 'festival', label: 'Festival', icon: '🎊', color: 'text-purple-400 bg-purple-500/20', tier: 2 },
    { key: 'event', label: 'Event', icon: '📅', color: 'text-blue-400 bg-blue-500/20', tier: 2 },
    { key: 'occasion', label: 'Occasion', icon: '🎈', color: 'text-pink-400 bg-pink-500/20', tier: 2 },
    { key: 'realization', label: 'Realization', icon: '💡', color: 'text-yellow-400 bg-yellow-500/20', tier: 2 },
    
    // Tier 3 - Detailed Information
    { key: 'timing', label: 'Timing', icon: '⏰', color: 'text-gray-400 bg-gray-500/20', tier: 3 },
    { key: 'time', label: 'Time', icon: '⏰', color: 'text-gray-400 bg-gray-500/20', tier: 3 },
    { key: 'method', label: 'Method', icon: '⚙️', color: 'text-blue-400 bg-blue-500/20', tier: 3 },
    { key: 'witnesses', label: 'Witnesses', icon: '👀', color: 'text-green-400 bg-green-500/20', tier: 3 },
    { key: 'surprise_element', label: 'Surprise', icon: '🎊', color: 'text-purple-400 bg-purple-500/20', tier: 3 },
    { key: 'logistics', label: 'Logistics', icon: '📋', color: 'text-gray-400 bg-gray-500/20', tier: 3 },
    { key: 'strategy', label: 'Strategy', icon: '🎯', color: 'text-purple-400 bg-purple-500/20', tier: 3 },
    { key: 'constraint', label: 'Constraint', icon: '⚠️', color: 'text-yellow-400 bg-yellow-500/20', tier: 3 },
    ...
    { key: 'memorable_event', label: 'Event Details', icon: '📝', color: 'text-blue-400 bg-blue-500/20', tier: 3 },
    { key: 'memorable_incident', label: 'Incident', icon: '📖', color: 'text-blue-400 bg-blue-500/20', tier: 3 },
    { key: 'memorable_items', label: 'Items', icon: '🎒', color: 'text-green-400 bg-green-500/20', tier: 3 },
    { key: 'comfort_items', label: 'Comfort Items', icon: '🧸', color: 'text-pink-400 bg-pink-500/20', tier: 3 },
    { key: 'pattern', label: 'Pattern', icon: '🔄', color: 'text-indigo-400 bg-indigo-500/20', tier: 3 },
    { key: 'flex', label: 'Flex', icon: '💪', color: 'text-orange-400 bg-orange-500/20', tier: 3 },
    { key: 'discovery', label: 'Discovery', icon: '🔬', color: 'text-green-400 bg-green-500/20', tier: 3 },
    { key: 'budget_notes', label: 'Budget', icon: '💰', color: 'text-yellow-400 bg-yellow-500/20', tier: 3 },
    { key: 'incident', label: 'Incident', icon: '📋', color: 'text-orange-400 bg-orange-500/20', tier: 3 },
    { key: 'attempted_dish', label: 'Attempted Dish', icon: '🍳', color: 'text-orange-400 bg-orange-500/20', tier: 3 },
    { key: 'successful_dish', label: 'Successful Dish', icon: '🍽️', color: 'text-green-400 bg-green-500/20', tier: 3 },
    { key: 'ingredients', label: 'Ingredients', icon: '🥕', color: 'text-green-400 bg-green-500/20', tier: 3 },
    { key: 'emotional_support', label: 'Support', icon: '💝', color: 'text-pink-400 bg-pink-500/20', tier: 3 },
    { key: 'care', label: 'Care', icon: '💝', color: 'text-pink-400 bg-pink-500/20', tier: 3 },
    { key: 'response', label: 'Response', icon: '💬', color: 'text-blue-400 bg-blue-500/20', tier: 3 },
    { key: 'description', label: 'Description', icon: '📄', color: 'text-gray-400 bg-gray-500/20', tier: 3 },
    { key: 'role', label: 'Role', icon: '🎭', color: 'text-purple-400 bg-purple-500/20', tier: 3 },
    { key: 'song_type', label: 'Song Type', icon: '🎶', color: 'text-purple-400 bg-purple-500/20', tier: 3 },
    
    // Common nested fields from details objects (flattened)
  ...
  // Get fields that have values for the current tier
  const getFieldsForTier = (tier: number) => {
    return allFields.filter(field => {
      if (field.tier > tier) return false;
      const value = event[field.key];
      return value && value !== '' && (Array.isArray(value) ? value.length > 0 : true);
    });
  };

  const currentFields = getFieldsForTier(showTier);
  const hasMoreTiers = getFieldsForTier(showTier + 1).length > currentFields.length;
    ...
    <div className="mt-4 space-y-2">
      {currentFields.map(field => {
        ...
        return (
          ...
          >
            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${field.color} flex-shrink-0`}>
      ...
      {hasMoreTiers && (
        <button
          onClick={() => setShowTier(prev => prev + 1)}
          className="text-xs text-purple-400 hover:text-purple-300 underline mt-2 flex items-center"
        >
          <Plus size={12} className="mr-1" />
          show more details ({getFieldsForTier(showTier + 1).length - currentFields.length} more)
        </button>
      )}
      
      {showTier > 1 && (
        <button
          onClick={() => setShowTier(1)}
          className="text-xs text-gray-400 hover:text-gray-300 underline mt-2 ml-4"
        >
          show less
        </button>
      )}
...
function Timeline() {
  ...
  useEffect(() => {
    const handleKeyNavigation = (e: KeyboardEvent) => {
      ...
      const currentEvents = searchTerm || selectedCategory ? filteredEvents : events;
  ...
  }, [showModal, showEditModal, showImageModal, events, filteredEvents, searchTerm, selectedCategory, activeIndex]);
            ...
            {events.length > 0 && (
              <div className="mt-6 flex flex-wrap justify-center gap-4 text-sm">
                <div className="bg-white/10 rounded-lg px-4 py-2 border border-white/10">
                  ...
                  <span className="text-white font-semibold ml-2">{events.length}</span>
                </div>
                <div className="bg-white/10 rounded-lg px-4 py-2 border border-white/10">
          ...
          {events.length === 0 ? (
            ...
            <div className={`${isMobile ? 'overflow-x-auto' : 'space-y-8'}`}>
              {(searchTerm || selectedCategory) && filteredEvents.length === 0 ? (
                ...
                <>
                  {isMobile ? (
                    <div className="flex space-x-6 pb-4" style={{ width: `${(searchTerm || selectedCategory ? filteredEvents : events).length * 300}px` }}>
                      {(searchTerm || selectedCategory ? filteredEvents : events).map((event, index) => (
                        <motion.div 
                          key={event.id} 
                    ...
                    (searchTerm || selectedCategory ? filteredEvents : events).map((event, index) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {index < (searchTerm || selectedCategory ? filteredEvents : events).length - 1 && (
                  ...
                  {!hasMorePages && (searchTerm || selectedCategory ? filteredEvents : events).length > 0 && (
